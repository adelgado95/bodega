...
<script>
 $(document).ready(function() {
 	...
 	$('#btnExportarEntrada').click(function () {
      ...
      location.href ="models/exports/exportsalidasfechas.php?fechai="+vfecha+"&fechaf="+vfecha2;
 	});
    ...
    function CargarSalidas(){
      ...
      var vfecha2 = $('#datepicker2').val();
           $.ajax({
            type:'GET',
            url:'models/salida.php?mode=reportsalidasfechas&fechai='+vfecha+'&fechaf='+vfecha2,
            success:function(datos){
             ...
             console.log(dat);
               $.each(dat,function(i,value){
                      table.row.add([dat[i].Imei,dat[i].Iccid,dat[i].Marca,dat[i].Modelo,dat[i].Precio,dat[i].PrecioTAE,dat[i].Fecha_Entrada,dat[i].Estado,dat[i].Fecha_Salida,dat[i].Zona,dat[i].Canal,dat[i].Encargado,dat[i].Liquidado]).draw();
